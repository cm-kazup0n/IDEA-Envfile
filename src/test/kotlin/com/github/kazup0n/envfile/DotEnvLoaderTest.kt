...
internal class DotEnvLoaderTest {
    ...
    fun testParser() {
        ...
        val lines = ManyParser(entry)

        fun loadFile(file: File) {
            try {
                val ctx = ParserContext.fromString(file.readText(Charsets.UTF_8))
                val result = lines.parse(ctx)
                val values = result.first.nodeList
                    .filter { it.node.astNodeName == "ValueNode" }
                    .associate {
                        (it.children[0] as ValueNode<Pair<String, String>>).value
                    }
            } catch (e: ParserException) {
                println(e.input.sourcePosition)
                throw e
            }
        }
        loadFile(File("/Users/sasaki.kazuhiro/src/github.com/cm-sbj/mop-app-api/.env"))
    }
}
